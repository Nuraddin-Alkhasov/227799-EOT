...
namespace HMI.Services
{
    ...
    public class Service_H_LiftTiltingDevice : ServiceBase, IArticle
    {
        ...
        protected override void OnLoadProjectCompleted()
        {
            ...
            AToPLC = VS.GetVariable("CPU1.PLC.Blocks.01 Basket feeding.01 LD.00 Main.DB LD HMI.PC.PD.Handshake.to PC.GetRecipe");
        ...
        async Task A_To_PLCAsync()
        {
            try
            {
                ...
                string Barcode = ApplicationService.GetVariableValue("CPU1.PLC.Blocks.01 Basket feeding.01 LD.00 Main.DB LD HMI.PC.PD.Data.to.Barcode").ToString();
                MR MR = new MR();
                if (Barcode == "0")
                {
                    uint MR_Id = (uint)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.01 Basket feeding.01 LD.00 Main.DB LD HMI.PC.PD.Header.MRId");
                    MR = GetMRData(MR_Id);
                    if (MR.Article.Header.Id == -1)
                    {
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.01 Basket feeding.01 LD.00 Main.DB LD HMI.PC.PD.Handshake.from PC.Not loaded", true);
                ...
                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.01 Basket feeding.01 LD.00 Main.DB LD HMI.PC.PD.Data.from.Layers", counter);

                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.01 Basket feeding.01 LD.00 Main.DB LD HMI.PC.PD.Data.from.Paint", MR.Layers[0].Paint_Id);
                
                //new NewLoad()
                //{
                //    MR = MR.Header.Name,
                //    MR_Id = VS.GetVariable("CPU1.PLC.Blocks.01 Basket feeding.01 LD.00 Main.DB LD HMI.PC.PD.Header.MRId")
                //}.SetMRIdToPLC();
                ...
                Application.Current.Dispatcher.Invoke(delegate
                {
                    ...
                    HMI.DialogRegion.MO.Views.MO_DataPicker v = (HMI.DialogRegion.MO.Views.MO_DataPicker)iRS.GetView("MO_DataPicker");
                    if (v.IsVisible) { ((HMI.DialogRegion.MO.Adapters.MO_DataPicker)v.DataContext).ViewUpdate(); }
                });
