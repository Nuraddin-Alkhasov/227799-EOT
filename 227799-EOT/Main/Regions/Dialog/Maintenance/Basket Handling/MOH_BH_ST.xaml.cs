        ...
        private void Reset_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@Maintenance.Text15", "@Maintenance.Text16", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                if (btn1.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.02 Basket handling.02 Basket Storage.DB ST Main HMI.Actual.Shutter inside.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.02 Basket handling.02 Basket Storage.DB ST Main HMI.Actual.Shutter inside.Operating hours.Reset", false);
                ...
                if (btn2.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.02 Basket handling.02 Basket Storage.DB ST Main HMI.Actual.Shutter outside.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.02 Basket handling.02 Basket Storage.DB ST Main HMI.Actual.Shutter outside.Operating hours.Reset", false);
