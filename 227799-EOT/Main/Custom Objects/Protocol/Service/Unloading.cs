...
namespace HMI.CO.Protocol
{
    class Unloading
    {
        ...
        private void WriteEndToOrders(Run r)
        {
            ...
            if ((bool)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.05 PO.01 PO Traction.DB PO Traction State.In Position.Unload 1"))
            {
                ApplicationService.SetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.05 PO.04 Unload 1.Belt.DB US 1 Belt HMI.PC.Order", Convert.ToInt32(T.Rows[0]["Data_1"].ToString()));
            }
            else 
            {
                ApplicationService.SetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.05 PO.05 Unload 2.Belt.DB US 2 Belt HMI.PC.Order", Convert.ToInt32(T.Rows[0]["Data_1"].ToString()));
            }
