...
namespace HMI.Services
{
    ...
    public class Service_H_Forplanet : ServiceBase, ICoating
    {
        ...
        protected override void OnLoadProjectCompleted()
        {
            ...
            CToPLC = VS.GetVariable("CPU1.PLC.Blocks.03 Coating.00 Main.DB Coating Main HMI.PC.Handshake.to PC.GetRecipe");
        ...
        async Task C_To_PLCAsync()
        {
            try
            {
                short IsLayer = (short)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.03 Coating.00 Main.DB Coating Main PD.Charge.Layers.Actual");

                if (IsLayer + 1 > 5)
                {
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.03 Coating.00 Main.DB Coating Main HMI.PC.Handshake.from PC.Not loaded", true);
                    new MessageBoxTask("@RecipeSystem.Results.Text8", "@RecipeSystem.Results.Text2", MessageBoxIcon.Error);
                }
                else
                {
                    long MR_Id = (uint)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.03 Coating.00 Main.DB Coating Main PD.Header.MRId");
                    ...
                    else
                    {
                        ...
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.03 Coating.00 Main.DB Coating Main PD.Charge.Paint.Actual", C_P_Actual);
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.03 Coating.00 Main.DB Coating Main PD.Charge.Paint.Next", C_P_Next);
